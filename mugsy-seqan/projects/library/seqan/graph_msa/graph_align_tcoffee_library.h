...
<<<<<<< .mine
		...
		for(int z = last - 1; z>=0; --z) {
				...
				(pos1[z].second == pos1[z+1].second + 1)) 
			{
			  ...
			  //SVA
			  //appendValue(matches, TFragment(positionToId(str, i),iBegin,positionToId(str, j),jBegin,lenMatch,lenMatch));
			...
			while (pos2 != posEnd) {
				if (pos->second != pos2->second) {
				  ...
				  //appendValue(matches, TFragment(positionToId(str, pos->second), pos->first, positionToId(str, pos2->second), pos2->first, ktup));
					appendValue(matches, TFragment(positionToId(str, pos->second), pos->first, positionToId(str, pos2->second), pos2->first, ktup,ktup));
