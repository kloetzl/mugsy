		    ...
		    FractionalScore)
{
	...
	//bool anno = false;
	//matchRefinement(matches,strSet,fake_score,outGraph,1,anno,ExactRefinement());
	//matchRefinement(matches,strSet,fake_score,outGraph,1,anno,InexactRefinement());
	//std::cout << "match refinement...";
	//std::cout.flush();
	matchRefinement(matches,strSet,outGraph);
	...
	//SVA need to preserve negative weights because I'm using them
	//to encode reverse strand matches
	bool revHack=true;
	for(;!atEnd(itE);goNext(itE)){
	  if(cargo(value(itE))<0 && revHack){
	    cargo(value(itE)) = -1;
	    assert(cargo(value(itE))<0);
	  }
	  else{
	    cargo(value(itE)) = 1;
	    assert(cargo(value(itE))>0);
	...
	for(; it != endIt; ++it, ++scoreIt) {
		...
		if(isReversed(*it)){
		  TId tid2;
		  getProjectedPosition(*it,id1,pos1,tid2,pos2);
		  assert(tid2==id2);
		}
		...
		while(pos1 < end1) {
			...
			//<<<<<<< .mine
			assert(p1 != getNil<TVertexDescriptor>());
			assert(p2 != getNil<TVertexDescriptor>());
			//implemented in graph_types/graph_impl_undirected.h
			TEdgeDescriptor e = findEdge(outGraph, p1, p2);
			TSize vertexLen = fragmentLength(outGraph, p1);
			if(e){
 			  assert(e!=0);
			  if(cargo(e) < 0 && revHack){
			    cargo(e) -= (TCargo) ((vertexLen * (*scoreIt)) / fragLen);
			    //(TCargo) (((double) fragmentLength(outGraph, p1) / (double) fragLen) * (double) value(scoreIt));
			  }
			  else{
			    cargo(e) += (TCargo) ((vertexLen * (*scoreIt)) / fragLen);
			    //(TCargo) (((double) fragmentLength(outGraph, p1) / (double) fragLen) * (double) value(scoreIt));
			  ...
			  //std::cerr << "MAPPED position:" << fragmentBegin(*it, id2) << " len:" << fragLen << std::endl;
			  //std::cerr << "Unable to find edge connecting vertex:" << p1 << "-" << p2 
			  //<< " S1:" << id1 << "," << pos1 
			  //<< " S2:" << id2 << "," << pos2 << std::endl;
			  assert(findEdge(outGraph,p2,p1)==0);
			  assert(e!=0);
			}
			...
			if(isReversed(*it)){
			  TId tid2;
			  if(pos1<end1){
			    assert(pos1-fragmentBegin(*it,id1)<fragmentLength(*it,id1));
			    getProjectedPosition(*it,id1,pos1,tid2,pos2);
			    assert(tid2==id2);
			  }
			  else{
...
_scoreMatches(StringSet<TString, TSpec> const& seqSet,
			  ...
			  String<Fragment<TSize, ExactReversableFragment<> >, TSpec2> const& matches,
			  TScoreString& scores,
			  TScoreValue offset)
{
	SEQAN_CHECKPOINT
	typedef String<Fragment<TSize, ExactReversableFragment<> >, TSpec2> TFragmentString;
...
{
	// ReScore
  //std::cout << "scoring matches..." << std::endl;
	_scoreMatches(stringSet(outGraph), scType, matches, scores);
	
	// Use fractinal score
	//std::cout << "building alignment graph..." << std::endl;
...
{
  tripletLibraryExtensionCond(g,true);
}

template<typename TStringSet, typename TCargo, typename TSpec>
inline void 
  tripletLibraryExtensionCond(Graph<Alignment<TStringSet, TCargo, TSpec> >& g, bool connectall=true)
{
	SEQAN_CHECKPOINT
	...
	for(;!atEnd(itE);goNext(itE)) {
		...
		//TCargo c = cargo(*itE);
		//SVA edit to support negative edge weights
		//c<0 match in opposing orientation
		//c>0 match in same orientation
		TCargo c = cargo(*itE);
	...
	for(; itEdges1 != itEdgesEnd; ++itEdges1) {
		for(TEdgeIter itEdges2 = itEdges1; ++itEdges2 != itEdgesEnd;) {
			...
			if (sequenceId(g, (*itEdges1).v2) != sequenceId(g, (*itEdges2).v2)) {
			  ...
			  TCargo weight = (abs((*itEdges1).c) < abs((*itEdges2).c)) ? abs((*itEdges1).c) : abs((*itEdges2).c);
			  if ((*itEdges1).v2 < (*itEdges2).v2) {
				...
				assert((*pos).second>0);
			      ...
			      if(connectall)
				newEMap.insert(std::make_pair(TNewEdge((*itEdges1).v2, (*itEdges2).v2), weight));
			    }
			  }
			  else {
				...
				assert((*pos).second>0);
			      ...
			      if(connectall)
				newEMap.insert(std::make_pair(TNewEdge((*itEdges2).v2, (*itEdges1).v2), weight));
			    }
			  }
			}
		}
...
template<typename TStringSet, typename TCargo, typename TSpec>
inline void 
  graphBasedTripletLibraryExtension(Graph<Alignment<TStringSet, TCargo, TSpec> >& g, bool connectall=true)
{
	SEQAN_CHECKPOINT
	typedef Graph<Alignment<TStringSet, TCargo, TSpec> > TGraph;
	typedef typename VertexDescriptor<TGraph>::Type TVertexDescriptor;
			...
			while (!atEnd(outIt2)) {
				...
				if (sequenceId(g, tV1) != sequenceId(g,tV2)) {
					...
					if (e == 0) {
					  if(connectall){
						// New edge
					    TCargo val = abs(cargo(*outIt1)) < abs(cargo(*outIt2)) ? abs(cargo(*outIt1)) : abs(cargo(*outIt2));
					  ...
					  if (abs(cargo(*outIt2)) > abs(cargo(*outIt1))){
					    if(cargo(e)<0){
					      property(newCargoMap, e) -= abs(cargo(*outIt1));
					    }
					    else{
					      property(newCargoMap, e) += abs(cargo(*outIt1));
					    }
					    ...
					    if(cargo(e)<0){
					      property(newCargoMap, e) -= abs(cargo(*outIt2));	
					    }
					    else{
					      property(newCargoMap, e) += abs(cargo(*outIt2));	
					    }
