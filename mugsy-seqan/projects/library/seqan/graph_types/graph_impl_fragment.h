...
template<typename TSpec = Default>
struct GappedFragment;	

template<typename TSpec = Default>
struct GappedReversableFragment;	

//////////////////////////////////////////////////////////////////////////////
// Default Fragment is the exact one
//////////////////////////////////////////////////////////////////////////////
 
//template<typename TSize = typename Size<String<char> >::Type, typename TSpec = SVAFragment<> >
// class Fragment;
 template<typename TSize = typename Size<String<char> >::Type, typename TSpec = ExactReversableFragment<> >
   class Fragment;
...
class Fragment<TSize, ExactFragment<TSpec> > {
 ...
 Fragment(TId sqId1, TSize beg1, TId sqId2, TSize beg2, TSize l, bool o ) :
	 seqId1(sqId1), begin1(beg1), seqId2(sqId2), begin2(beg2), len(l), reversed(o)
	 {
		 SEQAN_CHECKPOINT
	 }
					 ...
					 TPosition2& pos2)
{
	...
	if ((TId) seqId == f.seqId1) {
	  assert((TPosition1)f.begin1<=pos);
	  assert(pos - f.begin1 < f.len);
		SEQAN_TASSERT((TPosition1)f.begin1<=pos)
		  SEQAN_TASSERT(pos - f.begin1 < f.len)	
	  ...
	  assert(seqId == f.seqId2);
	  assert((TPosition1)f.begin2<=pos);
	  assert(pos - f.begin2 < f.len);
		SEQAN_TASSERT((TPosition1)f.begin2<=pos)
		SEQAN_TASSERT(pos - f.begin2 < f.len)
