...
struct MafFormat_;
typedef Tag<MafFormat_> const MafFormat;
...
struct MultiFastaFormat_;
typedef Tag<MultiFastaFormat_> const MultiFastaFormat;
...
//TCargo = int SVA allow for negative scores
template<typename TStringSet, typename TCargo = int, typename TSpec = Default>
struct Alignment;
...
/*
FragmentInfo(TId id, TSize beg, TSize len, bool orient) :
		data_seq_id(id),
		data_begin(beg),
data_length(len),
orient(orient)
      ...
      MsfFormat) 
{
	...
	if (convertAlignment(g, align)) {	
		...
		TSize offset = 0;
		for(TSize i = 0; i<nseq; ++i) {
		  //TSize currentSeq = idToPosition(value(g.data_sequence),i);
...
template <typename TFile, typename TSpec, typename TNames, typename TOffset, typename TSpans, typename TSeqlens, typename TOrients>
inline void
write(TFile & file,
      Graph<TSpec> const& g,
      TNames const& names,
  ...
  if(lcbid){}
	...
	if (convertAlignment(g, align)) {	
		...
		for(TSize i = 0; i<nseq; ++i) {
			...
			if(orients[i] == '-'){
			  //assert(seqlens[i]-(offsets[i]+spanlens[i])>=0);
			  _streamPutInt(file,seqlens[i]-(offsets[i]+spanlens[i]));
			  _streamPut(file, '-');
			  _streamPutInt(file,seqlens[i]-(offsets[i]+spanlens[i])+spanlens[i]);
			  _streamPut(file, ' ');
			  _streamPut(file, '-');
			  
			}
			else{
			  _streamPutInt(file,offsets[i]);
			  _streamPut(file, '-');
			  _streamPutInt(file,offsets[i]+spanlens[i]);
			  _streamPut(file, ' ');
			  _streamPut(file, '+');
			}
			_streamWrite(file," ");
			_streamWrite(file,"test.fsa");
			_streamPut(file,'\n');
			TSize col = 0;
			while(col < colLen) {

			  //text -- The nucleotides (or amino acids)
			  //in the alignment and any insertions
			  //(dashes) as well.

			  if (getValue(align, i*colLen + col) == '-'){
			    if(printGaps){
			      _streamPut(file, '-');
			    }
			  }
			  else _streamPut(file, getValue(align, i*colLen + col));
			  col++;
			}
			_streamPut(file, '\n');
		}
...
template <typename TSpec, typename TNames, typename TOffset, typename TSpans, typename TSeqlens, typename TOrients>
inline void
  write(std::string & fileprefix,
	Graph<TSpec> const& g,
	TNames const& names,
	MultiFastaFormat,
	TOffset & offsets,
	...
	bool printGaps=true)
{
  if(lcbid){}
	...
	if (convertAlignment(g, align)) {	
		...
		for(TSize i = 0; i<nseq; ++i) {
		  std::fstream file;
		  std::string fname(fileprefix+"_S"+boost::lexical_cast<std::string>(i)+".fsa");
		  ...
		  while(col < colLen) {
			...
			_streamPut(file, '-');
		      ...
		      _streamPut(file, getValue(align, i*colLen + col));
		    }
		    col++;
		  }
		  _streamPut(file, '\n');
...
write(TFile & file,
      ...
      TSeqlens & seqlens,
      TOrients & orients, 
      std::string label)//unsigned int lcbid)
{
	SEQAN_CHECKPOINT
	typedef Graph<TSpec> TGraph;
	typedef typename Size<TGraph>::Type TSize;
	...
	if (convertAlignment(g, align)) {	
		...
		_streamWrite(file, label.c_str());
		_streamWrite(file," mult=");
		...
		for(TSize i = 0; i<nseq; ++i) {
			...
			//size -- The size of the aligning region in
			//the source sequence. This number is equal to
			//the number of non-dash characters in the
			//alignment text field below.

			_streamPutInt(file,spanlens[i]);
			_streamWrite(file," ");

			//strand -- Either '+' or '-'. If '-', then
			//the alignment is to the reverse-complemented
			//source.
			file << orients[i];
			
			_streamWrite(file," ");
			_streamPutInt(file,seqlens[i]);
			_streamWrite(file," ");
			TSize col = 0;
			while(col < colLen) {
			  ...
			  if (getValue(align, i*colLen + col) == '-') _streamPut(file, '-');
			  else _streamPut(file, getValue(align, i*colLen + col));
			  col++;
			}
			_streamPut(file, '\n');
		     ...
		     )
{
	...
	if(current != (TEdgeStump*) 0) {
		...
		assert(false);
		if(cargo(current)<0){
		  std::cerr << "Using rev projection!!" << std::endl;
		  pos2 = (TPosition2) ((fragmentBegin(g,tV) + fragmentLen(g,sV) - 1) - (pos1 - fragmentBegin(g, sV)));
		}
		else{
		  std::cerr << "Using forward projection!!" << std::endl;
							...
							TOutString& align) 
{
	...
	//std::cout << "HCS matching positions" << numMatches << " alignLength:" << alignLength;
	clear(align);
	...
	while(pointerAlign != pointerAlignEnd) {
		...
		if (p>=0) {
			...
			/*
			//std::cout << "Matching positions " << i << "-" << j << std::endl;
			TSize str1len = length(str1[i]);
			for(TSize k=0;k<str1len;k++){
			  //std::cout << "V1:" << str1[i][k] << std::endl;
			}
			TSize str2len = length(str2[j]);
			for(TSize k=0;k<str2len;k++){
			  //std::cout << "V2:" << str2[j][k] << std::endl;
			}
			*/
		};
