...
{
  //SVA.  
  //Move location of the cut outside match area for reversed matches,
  //so we don't lose bases at the end of a match?
  if(f.reversed)
    ++pos_j;
}
//template<typename TSize, typename TSpec,typename TPos>
...
SEQAN_CHECKPOINT
	...
	for(unsigned int seq_i_pos = 0; seq_i_pos < length(seqs); ++seq_i_pos)
	{
		...
		{
			TValue pos_i = *it;	
			//std::cout << "REFINE GRAPH SEQ1:" << seq_i_id << " coord:" << pos_i 
			//<< " " << *next_it - pos_i << std::endl;
...
_makeRefinedGraphEdges(TAlignmentString & alis,
		       ...
		       TScore & score_type,
		       TAliGraph & ali_g,
		       Tag<TagExactRefinement_> const , bool revHack=false)
{
SEQAN_CHECKPOINT
	...
	while(ali_it != ali_end)
	{
		...
		while(act_pos < end_pos)
		{
			...
			//std::cout << "Fragment " << (*ali_it).seqId1 << "," << (*ali_it).seqId2
			//<< " coords:" << (*ali_it).begin1 << "," << (*ali_it).begin2 
			//<< " len:" << (*ali_it).len << std::endl;
			_getOtherSequenceAndProject(*ali_it,seq_map,seq_id,act_pos,seq_j_id,pos_j);
			//std::cout << "QUERY fragment " << act_knot << " qpos:"<< act_pos << " seqlen:" << length(getValueById(stringSet(ali_g), seq_id)) << std::endl;
			//std::cout << "MAPPED position:" << pos_j << " reversed:" << (*ali_it).reversed << " seqlen:" << length(getValueById(stringSet(ali_g), seq_j_id)) << std::endl;
			//find node that contains the projected position (pos_j)
			TVertexDescriptor vd = findVertex(ali_g, seq_j_id, pos_j);
			//std::cout << "Found fragment " << vd 
			//<< " with BEGIN:"<<fragmentBegin(ali_g,vd) << "LEN:" << fragmentLength(ali_g,vd) << std::endl;
			if((*ali_it).reversed){
			  SEQAN_TASSERT(fragmentBegin(ali_g,vd)+fragmentLength(ali_g,vd)-1==pos_j)
			    }
			else{
			  SEQAN_TASSERT(fragmentBegin(ali_g,vd)==pos_j)
			}
			
			//std::cout << "frags " << act_knot << "-" << vd << std::endl;
			//std::cout << "Lengths " << fragmentLength(ali_g,act_knot) << " " << fragmentLength(ali_g,vd) << std::endl;
	...
	//		typename Value<TScore>::Type score = fragmentLength(ali_g,vd);
		...
		//this needs to be generalized (makes sense for positive scores only)
			...
			if(score > 0)
			{
			  //SVA hack, set score to negative to designate reverse match
			  //Fragment<> segment = *ali_it;
			  if((*ali_it).reversed && revHack){
			    score = score*-1;
			    assert(score<0);
			  }
			  else{
			  ...
			  if (findEdge(ali_g, act_knot, vd) == 0){
			    //std::cout << "Added edge between " << act_knot << "-" << vd  << " with score " << score << std::endl;
			    addEdge(ali_g,act_knot,vd,(TCargo)score);
					...
					if((*ali_it).reversed && revHack){
					  if(getCargo(ed)>0){
					    //Inconsistent score
					    std::cout << "Inconsistent orientation between " 
						      << act_knot << "-" << vd 
						      << " edge score is: " << getCargo(ed) << " with alternative score: " << score << std::endl;
					  }
					  else{
					    //std::cout << "Updating score between " << act_knot << "-" << vd << " from " << getCargo(ed) << " to " << getCargo(ed)+score << " and score " << score << std::endl;
					    assignCargo(ed, getCargo(ed)+score);
					    assert(getCargo(ed)<0);
					  }
					}
					else{
					  if(getCargo(ed)<0){
...
SEQAN_CHECKPOINT
 ...
 start = clock();
	...
	//std::cout << "making annotations..." << std::endl;
