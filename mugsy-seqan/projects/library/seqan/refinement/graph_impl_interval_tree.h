...
SEQAN_CHECKPOINT
	...
	reserve(g.data_vertex,length(intervals)*2);
	reserve(pm,length(intervals)*2);
	...
	reserve(pm, length(pm), Generous());//Exact());
	reserve(g.data_vertex, length(g.data_vertex), Generous());//Exact());
...
SEQAN_CHECKPOINT
	...
	reserve(g.data_vertex,length(intervals)*2, Generous());
	reserve(pm,length(intervals)*2, Generous());
	TVertexDescriptor root = addVertex(g);
	resizeVertexMap(g,pm);
...
{
SEQAN_CHECKPOINT
  //IntervalAndCargo<int,long unsigned int>* checkOne = getValue(intervals,0); 
  //assert(length(intervals)>0);
  //std::cout <<leftBoundary(*(getValue(intervals,0)))<<"->" << knot << " " << center << " len:" << len << std::endl;
	if(len==1){
	...
	TValue min1 = 400000000;//100000000;//300000000;
	TValue min2 = 400000000;//100000000;//300000000;
	TValue max1 = 0;
	TValue max2 = 0;
...
SEQAN_CHECKPOINT
  //assert(length(intervals)>0);
  //IntervalAndCargo<int,ExactReversableFragment<>* >* checkOne = getValue(intervals,0); 
...
SEQAN_CHECKPOINT
	...
	TValue min = (TValue)400000000.0;//1000000000.0;//300000000.0;//maxTValue;
	TValue max = 0;
